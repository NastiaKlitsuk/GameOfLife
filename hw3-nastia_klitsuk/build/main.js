...
var _es6EventEmitter = __webpack_require__(38);
...
var IsAliveEmitter = function (_Emitter) {
    _inherits(IsAliveEmitter, _Emitter);

    function IsAliveEmitter() {
        _classCallCheck(this, IsAliveEmitter);

        return _possibleConstructorReturn(this, (IsAliveEmitter.__proto__ || Object.getPrototypeOf(IsAliveEmitter)).call(this));
    }

    _createClass(IsAliveEmitter, [{
        key: 'onCellStateChanged',
        value: function onCellStateChanged() {
            for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
                args[_key] = arguments[_key];
            }

            this.trigger.apply(this, ['update-life-state'].concat(args));
        }
    }]);

    return IsAliveEmitter;
}(_es6EventEmitter2.default);

var events = exports.events = new IsAliveEmitter();
...
var Cell = exports.Cell = function () {
    function Cell(rowIndex, columnIndex) {
        ...
        var isAlive = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
        ...
        _events.events.on('update-life-state', function () {
            if (_this._rowIndex === (arguments.length <= 0 ? undefined : arguments[0]) && _this._columnIndex === (arguments.length <= 1 ? undefined : arguments[1])) {
                _this._isAlive = arguments.length <= 2 ? undefined : arguments[2];
            }
        });
    }

    _createClass(Cell, [{
        key: 'updateNextGenerationStatus',
        value: function updateNextGenerationStatus() {
        ...
        key: 'lifeStatusChanged',
        get: function get() {
            return this._lifeStatusChanged;
        }
    }, {
        key: 'isAliveInNextGeneration',
        set: function set(value) {
            this._isAliveInNextGeneration = value;
        },
        get: function get() {
            return this._isAliveInNextGeneration;
        }
    }]);
...
var _weakMap = __webpack_require__(46);
...
var Game = exports.Game = function () {
    ...
    function Game(rowsCount, columnsCount) {
        ...
        for (var rowIndex = 0; rowIndex < this._rowsCount; rowIndex++) {
            this._board[rowIndex] = [];
            for (var columnIndex = 0; columnIndex < this._columnsCount; columnIndex++) {
                this._board[rowIndex][columnIndex] = new _cell.Cell(rowIndex, columnIndex);
            }
        }
    }
    ...
    _createClass(Game, [{
        key: '_isNeighbour',
        value: function _isNeighbour(rowIndex, columnIndex, rowNeighbour, columnNeighbour) {
        ...
        key: '_getAliveNeighboursCount',
        value: function _getAliveNeighboursCount(rowIndex, columnIndex) {
        ...
        key: '_isCellAliveInNextGeneration',
        value: function _isCellAliveInNextGeneration(isAlive, aliveNeighboursCount) {
        ...
        key: '_updateNextGeneartionLifeStatus',
        value: function _updateNextGeneartionLifeStatus() {
            for (var rowIndex = 0; rowIndex < this._rowsCount; rowIndex++) {
                for (var columnIndex = 0; columnIndex < this._columnsCount; columnIndex++) {
        ...
        key: 'buildNextGenertion',
        value: function buildNextGenertion() {
            for (var rowIndex = 0; rowIndex < this._rowsCount; rowIndex++) {
                for (var columnIndex = 0; columnIndex < this._columnsCount; columnIndex++) {
                    var aliveNeighboursCount = this._getAliveNeighboursCount(rowIndex, columnIndex);
                    var cell = this._board[rowIndex][columnIndex];
...
var _viewManager = __webpack_require__(82);

$(document).ready(function () {
    var ROWS_COUNT = 28;
    var COLUMNS_COUNT = 28;
    var gameBoard = new _game.Game(ROWS_COUNT, COLUMNS_COUNT);
    var viewManager = new _viewManager.ViewManager(ROWS_COUNT, COLUMNS_COUNT);
    ...
    var buttonStopGame = document.querySelector('#stop-game');
    buttonStopGame.addEventListener("click", function () {
        isStopGame = true;
    });

    var buttonResetGame = document.querySelector('#reset-game');
...
var ViewManager = exports.ViewManager = function () {
    function ViewManager(rowsCount, columnsCount) {
        var _this = this;
        ...
        (function () {
            ...
            var _loop = function _loop(i) {
                var row = $('<div class="div-table-row"></div>').appendTo(board);

                var _loop2 = function _loop2(j) {
                    $('<div class="div-table-col"></div>').appendTo(row);
                    var lastAddedCell = '#board .div-table-row:nth-child(' + (i + 1) + ') .div-table-col:nth-child(' + (j + 1) + ')';
                    $(lastAddedCell).click(function () {
                        if ($(lastAddedCell).css('background-color') === 'rgb(0, 0, 0)') {
                            ...
                            _events.events.onCellStateChanged(i, j, false);
                        } else {
                            $(lastAddedCell).css('background-color', 'rgb(0, 0, 0)');
                            _events.events.onCellStateChanged(i, j, true);
                        }
                    });
                };

                for (var j = 0; j < _this._columnsCount; j++) {
                    _loop2(j);
                }
            };

            for (var i = 0; i < _this._rowsCount; i++) {
                _loop(i);
            }
        })();
    }
    ...
    _createClass(ViewManager, [{
        key: 'drawBoard',
        value: function drawBoard(gameBoard) {
            for (var rowIndex = 0; rowIndex < this._rowsCount; rowIndex++) {
                for (var columnIndex = 0; columnIndex < this._columnsCount; columnIndex++) {
                    if (gameBoard.board[rowIndex][columnIndex].lifeStatusChanged) {
                        var nthCell = '#board .div-table-row:nth-child(' + (rowIndex + 1) + ') .div-table-col:nth-child(' + (columnIndex + 1) + ')';
